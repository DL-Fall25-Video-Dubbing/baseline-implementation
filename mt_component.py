...
class MTComponent:
    ...
    def batch_translate(
        ...
        print(f"Batch translating {len(texts)} texts...")
        ...
        print(f"\n✓ Batch translation complete: {len(translations)} texts")
        return translations
    ...
    def translate_with_alternatives(
        ...
        (Useful for manual selection - baseline paper mentions manual editing)
        ...
        Returns:
            List of translation alternatives
        """
        src_code = self.LANGUAGE_CODES[source_lang]
        tgt_code = self.LANGUAGE_CODES[target_lang]
        ...
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                ...
                num_beams=num_alternatives * 2, # Use more beams than sequences
                temperature=1.2,
                do_sample=True,
                top_k=50,
                top_p=0.95
        ...
        # Decode alternatives
        alternatives = self.tokenizer.batch_decode(
            outputs,
            skip_special_tokens=True
        )
    ...
    def back_translate(
        ...
        """
        Translate and back-translate to check quality
        ...
        Returns:
            Dictionary with original, translation, and back-translation
        """
        # Forward translation
        translation = self.translate(text, source_lang, target_lang)
        if isinstance(translation, list): translation = translation[0] # Ensure string

        # Back translation
        back_translation = self.translate(translation, target_lang, source_lang)
        if isinstance(back_translation, list): back_translation = back_translation[0] # Ensure string
        ...
        return {
            ...
            'direction': f"{source_lang} → {target_lang} → {source_lang}"
        }
    ...
    def estimate_translation_length(
        ...
        """
        Estimate translated length (for dubbing alignment)
        ...
        return {
            ...
            'char_ratio': len(translation) / max(len(text), 1),
            'word_ratio': len(translation.split()) / max(len(text.split()), 1)
        }
...
def merge_short_segments(
    ...
    """
    Merge very short segments for better translation context
    (Baseline paper segments into ~12 second utterances)

    Args:
        segments: List of segments
        min_words: Minimum words to keep segment separate
        max_duration: Maximum merged segment duration
    ...
    for seg in segments[1:]:
        ...
        words_count = len(current['text'].split())
        potential_duration = seg['end'] - current['start']

        if words_count < min_words and potential_duration < max_duration:
            # Merge
...
def adjust_translation_timing(
    segment: Dict,
    target_duration: float,
    method: str = "speed"
) -> Dict:
    """
    Adjust translation to match target duration
    (Baseline paper mentions this is needed for dubbing)

    Args:
        segment: Translated segment
        target_duration: Target duration in seconds
        method: 'speed' (adjust playback) or 'trim' (shorten text)

    Returns:
        Adjusted segment with speed factor or trimmed text
    """
    ...
    if method == "speed":
        ...
        if target_duration == 0: target_duration = 0.1 # Avoid division by zero
        
        speed_factor = current_duration / target_duration
        segment['speed_adjustment'] = round(speed_factor, 2)

        if abs(speed_factor - 1.0) > 0.2:  # >20% speed change
            segment['speed_warning'] = True

    elif method == "trim":
        # Trim text if too long (simple word-based)
        length_ratio = segment.get('length_ratio', 1.0)
        if length_ratio > 1.2:  # 20% longer
            words = segment['translated_text'].split()
            target_words = int(len(words) / length_ratio)
            segment['translated_text'] = ' '.join(words[:target_words]) + '...'
            segment['trimmed'] = True

    return segment


def export_translation_to_json(
    segments: List[Dict],
    output_path: str,
    include_metadata: bool = True
):
    """
    Export translation results to JSON

    Args:
        segments: Translated segments
        output_path: Path to save JSON file
        include_metadata: Include timing and metadata
    """
    if include_metadata:
        output = {
            'segments': segments,
            'metadata': {
                ...
                'source_lang': segments[0]['source_lang'] if segments else None,
                'target_lang': segments[0]['target_lang'] if segments else None,
                'total_duration': segments[-1]['end'] if segments else 0,
                'avg_length_ratio': np.mean([s.get('length_ratio', 1.0) for s in segments]) if segments else 0
            }
        }
    else:
        output = segments
    ...
    print(f"✓ Translation exported: {output_path}")


def export_translation_to_txt(
    segments: List[Dict],
    output_path: str,
    include_original: bool = True
):
    """
    Export translation to plain text file
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        for seg in segments:
            if include_original:
                f.write(f"[{seg['id']}] ORIGINAL ({seg['source_lang']}):\n")
                f.write(f"{seg['original_text']}\n\n")
                f.write(f"[{seg['id']}] TRANSLATION ({seg['target_lang']}):\n")
...
def print_translation_summary(segments: List[Dict]):
    """
    Pretty print translation summary

    Args:
        segments: Translated segments
    """
    if not segments:
        print("No segments to summarize.")
        return
    ...
    print(f"Direction: {segments[0]['source_lang'].upper()} → {segments[0]['target_lang'].upper()}")

    # Length statistics
    ratios = [s.get('length_ratio', 1.0) for s in segments]
    print(f"\nLength Ratios:")
    ...
    # Warnings
    warnings = sum(1 for s in segments if s.get('length_warning', False))
    if warnings > 0:
        print(f"\n⚠ Length Warnings: {warnings}/{len(segments)} segments")
    ...
    for seg in segments[:3]:
        ...
        print(f"  ORIGINAL: {seg['original_text'][:100]}...")
        print(f"  TRANSLATED: {seg['translated_text'][:100]}...")
        print(f"  Ratio: {seg.get('length_ratio', 1.0):.2f}")